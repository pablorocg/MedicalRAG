...
class OllamaEmbeddingProvider:
    ...
    def __init__(self, model_name: str = "nomic-embed-text", base_url: str = "http://localhost:11434"):
        ...
        self.lc_embeddings = OllamaEmbeddings(
            model=model_name,
            base_url=base_url
        )
...
if __name__ == "__main__":
    ...
    if not text_generator.client.check_connection():
        print("Error: No se puede conectar a Ollama. Verifique que el servicio esté en ejecución.")
        exit(1)
    
    print(f"Modelos disponibles: {json.loads(requests.get('http://localhost:11434/api/tags').text)['models']}")
    ...
    # Generar embeddings
    documents = ["La diabetes es una enfermedad crónica.", "La diabetes tipo 2 es la más común."]
    embeddings = embedding_provider.embed_documents(documents)
    
    print(f"Embeddings generados: {len(embeddings)} embeddings con {len(embeddings[0])} dimensiones cada uno")
