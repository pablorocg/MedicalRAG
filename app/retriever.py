...
class DocumentProcessor:
    ...
    def prepare_documents(self, dataset: TextDataset, max_samples: int = None) -> Tuple[List[str], List[Dict[str, Any]]]:
        """
        ...
        if max_samples and max_samples < len(dataset):
            ...
            print(f"Procesando un subconjunto de {max_samples} documentos de {len(dataset)}")
        else:
    ...
    def create_langchain_documents(self, dataset: TextDataset, max_samples: int = None) -> List[Document]:
        """
...
class ChromaVectorStore:
    ...
    def create_from_documents(self, documents: List[Document], batch_size: int = 128) -> None:
        """
        ...
        if len(documents) > batch_size:
            ...
            # Procesar en lotes
            for i in tqdm(range(0, len(documents), batch_size), desc="Procesando lotes de documentos"):
        ...
        print(f"Base de datos vectorial creada y persistida en {self.persist_directory}")
    ...
    def load(self) -> bool:
        ...
        if not os.path.exists(self.persist_directory):
            print(f"No se encontró una base de datos vectorial en {self.persist_directory}")
            return False
        ...
        try:
            ...
            if not os.path.exists(chroma_files[0]):
                print(f"No se encontraron archivos de ChromaDB en {self.persist_directory}")
                return False
...
class RAGRetriever:
    ...
    def _format_retrieved_context(self, documents: List[Document]) -> str:
        ...
        for i, doc in enumerate(documents, 1):
            ...
            formatted_context += f"Fuente: {doc.metadata.get('source', 'No disponible')}\n\n"
