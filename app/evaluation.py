...
def calculate_retrieval_metrics(
    ...
    if not query_results:
        return {"avg_retrieval_time": 0.0, "avg_num_docs": 0.0}
...
def calculate_generation_metrics(
    ...
    """
    if not query_results:
        return {"avg_generation_time": 0.0, "avg_answer_length": 0.0}
...
def calculate_content_metrics(
    generated_answers: List[str], reference_answers: List[str]
) -> Dict[str, float]:
    """
    ...
    rouge_scores = [
        scorer.score(ref, gen) for ref, gen in zip(reference_answers, generated_answers)
    ]
...
def run_evaluation(rag_components: Dict[str, Any], settings: Settings) -> None:
    ...
    for i, (query, reference) in enumerate(
        tqdm(zip(questions, reference_answers), total=max_samples)
    ):
    ...
    save_results_to_json(
        {"metrics": all_metrics, "query_results": query_results}, results_file
    )
    ...
    print(
        f"  Tiempo promedio de recuperación: {retrieval_metrics['avg_retrieval_time']:.4f} s"
    )
    print(
        f"  Promedio de documentos recuperados: {retrieval_metrics['avg_num_docs']:.2f}"
    )

    print("\nMétricas de generación:")
    print(
        f"  Tiempo promedio de generación: {generation_metrics['avg_generation_time']:.4f} s"
    )
    print(
        f"  Longitud promedio de respuesta: {generation_metrics['avg_answer_length']:.2f} palabras"
    )
